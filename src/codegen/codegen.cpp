...
const std::vector<std::string> kRuntimeFiles = {
    "rk4.hpp",          "csv_writer.hpp",     "plot_types.hpp",
    "plot_font.hpp",    "png_encoder.hpp",    "plot_framebuffer.hpp",
    "plot_mapping.hpp", "plot_render.hpp",
};
...
GeneratedFiles generateProject(const TranslationPlan &plan, std::string *err) {
  ...
  // ---- 绘图开关解析（contracts/plot-cli-contract.md）----
  body += "  mcruntime::plot::PlotRequest __plotReq;\n";
  body += "  for (int i = 1; i < argc; ++i) {\n";
  body += "    const std::string a = argv[i];\n";
  body += "    if (a == \"--plot\") __plotReq.enabled = true;\n";
  body += "    else if (a.rfind(\"--plot-columns=\", 0) == 0) {\n";
  body += "      std::string list = a.substr(15);\n";
  body += "      for (std::size_t p = 0; p < list.size();) {\n";
  body += "        const std::size_t c = list.find(',', p);\n";
  body += "        __plotReq.columns.push_back(list.substr(p, c == std::string::npos ? c : c - p));\n";
  body += "        if (c == std::string::npos) break;\n        p = c + 1;\n      }\n";
  body += "    }\n";
  body += "    else if (a.rfind(\"--plot-output=\", 0) == 0) __plotReq.outputPath = a.substr(14);\n";
  body += "  }\n\n";
  ...
  // 绘图采集缓冲（先于 writeRow 声明）
  body += "    std::vector<double> __pt;\n";
  for (const auto &var : plan.outputOrder) {
    body += "    std::vector<double> __pv_" + var.cppName + ";\n";
  }
  body += "\n";
  ...
  body += "});\n";
  // 绘图数据采集（与 CSV 同源）
  body += "      __pt.push_back(rowTime);\n";
  for (const auto &var : plan.outputOrder) {
    body += "      __pv_" + var.cppName + ".push_back(static_cast<double>(s." +
            var.cppName + "));\n";
  }
  body += "    };\n\n";
  ...
  // ---- 绘图输出（FR-001/002/005；失败不改变退出码）----
  body += "    if (__plotReq.enabled) {\n";
  body += "      std::vector<mcruntime::plot::TimeSeries> __series;\n";
  body += "      __series.reserve(" + std::to_string(plan.outputOrder.size()) + ");\n";
  for (const auto &var : plan.outputOrder) {
    body += "      __series.push_back({\"" + var.sourceName + "\", __pt, __pv_" +
            var.cppName + "});\n";
  }
  body += "      if (__plotReq.outputPath.empty()) __plotReq.outputPath = \"" + m +
          "_result.png\";\n";
  body += "      auto __ps = mcruntime::plot::render_and_save(\"" + m +
          "\", __series, __plotReq);\n";
  body += "      if (__ps.ok) {\n";
  body += "        std::printf(\"plot: %s\\n\", __ps.outputPath.c_str());\n";
  body += "        if (__ps.skippedPoints > 0)\n";
  body += "          std::fprintf(stderr, \"[plot] 已跳过 %ld 个非有限数据点\\n\", "
          "__ps.skippedPoints);\n";
  body += "      } else {\n";
  body += "        std::printf(\"plot: skipped (%s)\\n\", __ps.skipReason.c_str());\n";
  body += "        if (!__ps.detail.empty())\n";
  body += "          std::fprintf(stderr, \"[plot] %s\\n\", __ps.detail.c_str());\n";
  body += "      }\n";
  body += "    }\n";
  body += "    return 0;\n";
  body += "  } catch (const std::runtime_error& e) {\n";
  body += "    if (__plotReq.enabled) {\n";
  body += "      std::printf(\"plot: skipped (simulation diverged)\\n\");\n";
  body += "      std::fprintf(stderr, \"[plot] 放弃绘图：仿真发散\\n\");\n";
  body += "    }\n";
